...
function SpawnCar()
	ESX.TriggerServerCallback('NX_carsmuggling:isActive', function(isActive, cooldown)
		if cooldown <= 0 then
			if isActive == 0 then
				ESX.TriggerServerCallback('NX_carsmuggling:anycops', function(anycops)
					if anycops >= Config.CopsRequired then
						...
						TriggerServerEvent('NX_carsmuggling:registerActivity', 1)
...
function FinishDelivery()
  if(GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) and GetEntitySpeed(car) < 3 then
    ...
    --Pay the poor fella
		...
		TriggerServerEvent('NX_carsmuggling:pay', finalpayment)

		--Register Activity
		TriggerServerEvent('NX_carsmuggling:registerActivity', 0)
    ...
    TriggerServerEvent('NX_carsmuggling:stopalertcops')
...
Citizen.CreateThread(function()
  while true do
    ...
    if isTaken == 1 and IsPedInAnyVehicle(GetPlayerPed(-1)) then
      ...
      TriggerServerEvent('NX_carsmuggling:alertcops', coords.x, coords.y, coords.z)
...
RegisterNetEvent('NX_carsmuggling:removecopblip')
AddEventHandler('NX_carsmuggling:removecopblip', function()
		RemoveBlip(copblip)
end)

RegisterNetEvent('NX_carsmuggling:setcopblip')
AddEventHandler('NX_carsmuggling:setcopblip', function(cx,cy,cz)
		RemoveBlip(copblip)
...
RegisterNetEvent('NX_carsmuggling:setcopnotification')
AddEventHandler('NX_carsmuggling:setcopnotification', function()
	ESX.ShowNotification(_U('car_stealing_in_progress'))
end)

AddEventHandler('NX_carsmuggling:hasEnteredMarker', function(zone)
...
AddEventHandler('NX_carsmuggling:hasExitedMarker', function(zone)
	CurrentAction = nil
...
Citizen.CreateThread(function()
  while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('NX_carsmuggling:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('NX_carsmuggling:hasExitedMarker', LastZone)
		end
	end
end)
